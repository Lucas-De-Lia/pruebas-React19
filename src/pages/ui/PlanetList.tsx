...
export const PlanetList = ({ planets }: Props) => {
  ...
  const [optimisticPlanets, setOptimisticPlanets] = useOptimistic(planets,
    (current, newPlanet: Planet) => {
      const updatedPlanets = current.map((planet) =>
        planet.id === newPlanet.id
          ? { ...planet, ...newPlanet }
          : planet
      );
  ...
  const handleUpdatePlanet = async (planet: Planet) => {
    startTransition(async () => {
      ...
      try {
        ...
        const newPlanet = await updatePlanetAction(data);
        setOptimisticPlanets(newPlanet);
      } catch (error) {
